...
function Home() {
  ...
  const [user, setUser] = useState("user"+Math.floor(Math.random() * 10));
  ...
  useEffect(() => {
    ...
    if (socket === undefined) {
      var newSocket: Socket = io("ws://localhost:5000");
        ...
        <div className="flex flex-col justify-center items-center w-full">
          {messages.length > 0 &&
            messages.map((message, key) => {
              return (
                <div key={key} className="flex w-1/2">
                  {message.sender == user ? (
...
//   const socket = await fetch("http://localhost:3000/api/socket")
//   console.log(socket);
//   const result = await socket.json()
//   return {props:{ result}}
// };
export default Home;
